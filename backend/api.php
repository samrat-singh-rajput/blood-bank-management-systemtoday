...
class BloodBankAPI {
    ...
    private function sendOTP() {
        ...
        if (!$phone || !$email) { echo json_encode(["error" => "Phone number and email required"]); return; }

        // Check if phone or email already registered and verified
        $check = $this->db->query("SELECT id FROM users WHERE (phone=? OR email=?) AND is_verified=1", [$phone, $email], "ss");
        if ($check->num_rows > 0) {
            echo json_encode(["error" => "Mobile number or Email already registered."]); return;
        }
        ...
        if ($existing->num_rows > 0) {
            $this->db->execute("UPDATE users SET otp=?, otp_expiry=DATE_ADD(NOW(), INTERVAL 5 MINUTE), email=? WHERE phone=?", [$otp, $email, $phone], "sss");
        } else {
            $id = uniqid('PENDING_');
            $cleanPhone = preg_replace('/[^0-9]/', '', $phone);
            $pendingUser = "pending_" . $cleanPhone;
            $this->db->execute("INSERT INTO users (id, phone, otp, otp_expiry, status, is_verified, role, username, password, name, email) VALUES (?, ?, ?, DATE_ADD(NOW(), INTERVAL 5 MINUTE), 'Pending', 0, 'USER', ?, '', 'Pending User', ?)", [$id, $phone, $otp, $pendingUser, $email], "sssss");
        }
        ...
        if ($mailResult) {
            ...
            echo json_encode(["error" => "Failed to send OTP to email."]);
        }
    }
